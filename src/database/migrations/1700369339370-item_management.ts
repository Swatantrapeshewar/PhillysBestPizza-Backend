...
export class ItemManagement1700369339370 implements MigrationInterface {
	...
	public async up(queryRunner: QueryRunner): Promise<void> {
		...
		await queryRunner.query(
			`CREATE TABLE \`Items\` (\`id\` varchar(64) NOT NULL, \`name\` varchar(64) NOT NULL, \`description\` varchar(256) NULL, \`image\` text NULL, \`dailyThreshold\` bigint NULL, \`weeklyThreshold\` bigint NULL, \`overallThreshold\` bigint NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`categoryId\` varchar(64) NULL, UNIQUE INDEX \`REL_030ccb84c762ca5597340d19f1\` (\`categoryId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
		);
		await queryRunner.query(
